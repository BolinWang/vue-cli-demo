<template>
    <div ref="video-wrap" class="video-wrap">
        <template v-if="src">
            <div class="custom-detail-video-wrap" :style="tStyle" v-video>
                ...
                <div class="custom-detail-video-controls" v-show="showControls">
                    ...
                    <div class="fullscreen-btn" :class="`fullscreen-btn-${progressType || '1'}`" v-show="duration"></div>
                </div>  
                ...
                <div class="custom-detail-video-left-progress" :class="`progressType${progressType || '1'}`">
                <!-- @touchstart.passive="touchStart"
                    @touchmove.stop.prevent="touchMove"
                    @touchend.passive="touchEnd"
                    @touchcancel.passive="touchEnd" -->
                    <div>
                        <div v-show="(showControls) && duration" class="custom-detail-video-left-bg-progress">
...
import { requestFullscreen } from '@/tools/fullscreen-api'

export default {
    props: ['src', 'uniqueId', 'tStyle', 'poster', 'preload', 'autoplay', 'loop', 'isReverse', 'scale', 'useH5', 'progressType'],
    data(){
        return {
            ...
            pausePoster: '',
            x5videoenterfullscreen: false,
            androidPlayIsError: false,
        };
    },
    computed: {
        phoneIsAndroid(){
            return system.phoneIsAndroid();
        },
        video(){
        ...
        video: {
            inserted: function(el, binding, vnode) {
                let self = vnode.context,
                    ...
                    oFullscreenBtn = el.querySelector('.fullscreen-btn');

                let currentTimeMoveTimer = null, 
                    autoHideControlsTimer = null,
                    checkAndroidPlayIsErrorTimer = null;
                ...
                (function(oFullscreenBtn, oVideo){
                    self.oFullscreenBtn = oFullscreenBtn;
                    self.event_bind(oFullscreenBtn, 'click', (e)=>{
                        //取消默认行为和冒泡
                        e.preventDefault();
                        e.stopPropagation();

                        requestFullscreen(oVideo);
                    });
                })(oFullscreenBtn, oVideo);
                ...
                (function(self, oVideo){
                    ...
                    self.event_bind(oVideo, "canplay", ()=>{
                        self.waiting = false;
                    });

                    self.event_bind(oVideo, "canplaythrough", ()=>{
                        self.waiting = false;
                    });
                    ...
                    self.event_bind(oVideo, "play", (e)=>{
                        ...
                        if(self.phoneIsAndroid && !self.androidPlayIsError){
                            //开启延时器去判断是否播放出错了
                            checkAndroidPlayIsError();
                        }
                    ...
                    self.event_bind(oVideo, "pause", ()=>{
                        ...
                        //如果是android需要获取当前桢的图片
                        if(self.phoneIsAndroid){
                            self.pausePoster = self.src + `?x-oss-process=video/snapshot,t_${parseInt(oVideo.currentTime * 1000)},m_fast`
                        }
                ...
                (function(self, oPoint, oVideo, oProgress){
                    ...
                    function touchMoveFn(e){
                        ...
                        let { changedTouches: [{ clientX, clientY }] } = e;
                        let left = getLeft({ clientX, clientY });
                        oPlayProgress.style.width = oPoint.style.left = left + 'px';
                        self.currentTime = (getLeft({ clientX, clientY }) / oProgress.offsetWidth) * self.duration;
                    }
                    ...
                    function touchEndFn(e){
                        ...
                        let left = getLeft({ clientX, clientY });
                        oPlayProgress.style.width = oPoint.style.left = left + 'px';
                        self.currentTime = oVideo.currentTime = (getLeft({ clientX, clientY }) / oProgress.offsetWidth) * self.duration;
                    ...
                    function getLeft({ clientX, clientY }){
                        let clientXOrY = self.isReverse && self.isReverse() ? clientY : clientX;
                        let left = clientXOrY - ($.getOffsetLeft(oPoint, self.scale) - oPoint.offsetLeft);
                        if(left < 0) left = 0;
                ...
                if(self.phoneIsAndroid){
                    (function(self, oVideo){
                        ...
                        self.event_bind(oVideo, "x5videoenterfullscreen", ()=>{
                            
                            self.x5videoenterfullscreen = true;
                            clearTimeout(checkAndroidPlayIsErrorTimer);

                            if(self.useH5){
                                oVideo.className = 'richtext-detail-x5videoenterfullscreen-video';
                        ...
                        self.event_bind(oVideo, "x5videoexitfullscreen", ()=>{
                            
                            self.x5videoenterfullscreen = false;
                ...
                function checkAndroidPlayIsError(){
                    clearTimeout(checkAndroidPlayIsErrorTimer);
                    checkAndroidPlayIsErrorTimer = setTimeout(()=>{
                        console.log('androidPlayIsError=true');
                        //说明视频已经正常进入全屏播放模式
                        // if(!self.x5videoenterfullscreen){
                            self.androidPlayIsError = true;
                        // }
                    }, 1000);
                }
                    ...
                    if(wx){
                        wx.getNetworkType({
                            success: function (res) {
                                ...
                                if(networkType != 'wifi'){
                                    try{
                                        self.actionVuexMessageShow('当前处于移动数据网络');
                                    }catch(e){
                                        console.error(e);
                                    }
                                }
            ...
            unbind(el, binding, vnode){
                ...
                self.event_unbindAll(self.oFullscreenBtn);
            }
        }
    }
}
    ...
    .custom-detail-video-controls{
        ...
        .btn-group{
            ...
            span.play-btn{
                ...
                height:.4rem;
                margin: 0 auto;
                background: url(../file/video/play-btn.png) no-repeat center center;
                background-size:100%;
            }
            span.pause-btn{
                display: block;
                ...
                height:.4rem;
                margin: 0 auto;
                background: url(../file/video/pause-btn.png) no-repeat center center;
            ...
            span.waiting-btn{
                ...
                height:.4rem;
                margin: 0 auto;
                background: url(../file/video/waiting-btn.gif) no-repeat center center;
                background-size:100%;
            }
            p{
                margin-top:.125rem;
        ...
        .fullscreen-btn{
            ...
            right:.145rem;
            bottom:.39rem;
            background: url(../file/video/fullscreen-btn.png) no-repeat center center;
            background-size:100%;
            &:before{
                content: '';
                position: absolute;
